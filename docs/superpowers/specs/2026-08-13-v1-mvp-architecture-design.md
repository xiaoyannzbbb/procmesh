...
Join 是可恢复的多阶段操作。`join_prepare` 已提交而后续失败时不得退款或再次消费 token；相同 token、node ID、Raft 地址、CSR 与 operation ID 的重试复用原 attempt 和证书。operation ID 在所有 Join attempt 间全局唯一，不得用同一 operation ID 为另一个节点再次消费多用途 token。加入节点在本地用 `0600` 原子保存 pending CSR/private key，跨 CLI 调用复用，并在成功或确定性拒绝后删除；pending 文件不保存 token 明文。Leader 启动时及此后每 5 秒对账 FSM 与 Raft configuration：缺失的 `ADMITTED/JOINING` 成员作为 nonvoter 补入；地址不一致时保留该成员当前 voter/nonvoter 身份进行纠正；只有精确读回 node ID、Raft 地址和有效角色后才完成 `JOINING`。FSM 中已明确为 `REMOVED/REVOKED` 但仍在 configuration 的成员会继续重试移除。单个成员失败不能阻塞同轮其它独立成员，最终错误合并返回。Join、remove、promote、自动对账和手工对账必须使用同一 membership 串行化边界，并且失去 Leader 或 quorum 时停止写入。Raft command、日志和快照只保存 join token 哈希，不保存明文。成员变更失败必须返回稳定错误；若错误结果不确定，先读回 configuration，只有目标状态精确匹配时才按成功继续。
