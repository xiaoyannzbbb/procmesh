...
Join 是可恢复的多阶段操作。`join_prepare` 已提交而后续失败时不得退款或再次消费 token；相同 token、node ID、Raft 地址、CSR 与 operation ID 的重试复用原 attempt 和证书。operation ID 在所有 Join attempt 间全局唯一，不得用同一 operation ID 为另一个节点再次消费多用途 token。加入节点在本地用 `0600` 原子保存 pending CSR/private key，跨 CLI 调用复用，并在成功或确定性拒绝后删除；pending 文件不保存 token 明文。Raft advertise 必须是可拨号的 `host:port`，不得使用 `0.0.0.0`、`::` 或端口 `0`；加入节点在创建 pending identity 前检查，Leader 在消费 token 和创建 attempt 前重新检查。生产 Raft transport 还必须拒绝拨号到旧版本已经持久化的 wildcard 地址，防止目标被内核解释为本地 listener 并触发 Leader 自降级。Leader 启动时及此后每 5 秒对账 FSM 与 Raft configuration：缺失的 `ADMITTED/JOINING` 成员作为 nonvoter 补入；地址不一致时保留该成员当前 voter/nonvoter 身份进行纠正；只有精确读回 node ID、Raft 地址和有效角色后才完成 `JOINING`。缺失或 wildcard Raft 地址必须报告为不可自动修复，不能补入或推进 `JOINING`。FSM 中已明确为 `REMOVED/REVOKED` 但仍在 configuration 的成员会继续重试移除。单个成员失败不能阻塞同轮其它独立成员，最终错误合并返回。Join、remove、promote、自动对账和手工对账必须使用同一 membership 串行化边界，并且失去 Leader 或 quorum 时停止写入。Raft command、日志和快照只保存 join token 哈希，不保存明文。成员变更失败必须返回稳定错误；若错误结果不确定，先读回 configuration，只有目标状态精确匹配时才按成功继续。

FSM 是成员关系的权威期望状态，但对账采取保守删除策略：Raft configuration 中存在、FSM 中完全未知的成员只报告为 `UNEXPECTED_MEMBER/BLOCKED`，绝不自动删除，避免因 FSM 损坏或恢复错误破坏 quorum。`procmesh cluster membership check` 需要 `cluster.read`，返回带 `LIVE/STALE` 新鲜度且不含 Raft 地址的确定性报告；`procmesh cluster membership reconcile` 需要 `cluster.manage` 和 `operation_id`，经内部 mTLS 转发到 Leader 并在执行节点重新鉴权、审计，复用与后台任务相同的幂等逻辑。非 `CLEAN` CLI 结果返回非零退出码。待处理问题数、最后成功时间和连续失败次数以无标签 Prometheus 指标暴露。
