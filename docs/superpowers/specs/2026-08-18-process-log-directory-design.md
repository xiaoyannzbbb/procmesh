...
按大小轮转时先把活动文件复制为临时归档，关闭后原子替换 `.1`，再原地截断活动文件。不得 rename 活动文件后另建空文件，因为运行中的业务进程持有启动时继承的 stdout / stderr fd，rename 会让后续输出继续写入 `.1`，而 tail / stream / download 读取空的新文件。原地截断保留 inode 和现有 fd；Follow 通过进程内轮转代次重置读取 offset。

copy-truncate 无法把不配合的业务进程写入与轮转做成原子事务。复制追上 EOF 到截断之间的极短窗口内，并发日志可能丢失少量字节；V1 接受该限制，不通过暂停业务进程或让 shim 用 pipe 接管 stdout 来换取强一致轮转。
