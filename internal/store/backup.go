...
const backupCols = `snapshot_id, cluster_id, node_id, created_at, process_ids_json,
		revision_range_json, sha256, bytes, sink, destination_profile, location, source_node_id, run_id, task_id, policy_id`
...
func (s *Store) PutBackup(ctx context.Context, rec BackupRecord) error {
	...
	_, err = s.db.ExecContext(ctx, `
		...
		string(pids), rec.RevisionRangesJSON, rec.SHA256, rec.Bytes, rec.Sink, rec.DestinationProfile, rec.Location, rec.SourceNodeID,
...
func scanBackup(row *sql.Row) (BackupRecord, error) {
	...
	err := row.Scan(
		...
		&rec.RevisionRangesJSON, &rec.SHA256, &rec.Bytes, &rec.Sink, &rec.DestinationProfile, &rec.Location, &rec.SourceNodeID,
		&rec.RunID, &rec.TaskID, &rec.PolicyID,
	)
